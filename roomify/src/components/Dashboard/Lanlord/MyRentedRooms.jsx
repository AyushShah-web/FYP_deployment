    ...
    const fetchRooms = async () => {
      try {
        ...
        if (response.status !== 200) {
          showToast("error", "Error occurred while fetching rented rooms");
          return;
        }

        console.log("Landlord Rented Rooms:", response.data.data);
          ...
          <table className="table-auto w-full border-collapse">
            ...
            <tbody>
              {rooms.map((room, index) => {
                if (!room.room) return null; // Skip broken entries
